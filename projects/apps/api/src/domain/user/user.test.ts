...
import { User, UserId, UserCreatedEvent, UserEmailChangedEvent } from './user.js';
...
describe('User', () => {
  ...
  describe('changeName', () => {
    ...
    it('should fail when name exceeds 100 characters', () => {
      ...
      const result = user.changeName('a'.repeat(101));
    ...
    it('should succeed with boundary values', () => {
      ...
      expect(user.changeName('A').isSuccess()).toBe(true);
      expect(user.changeName('a'.repeat(100)).isSuccess()).toBe(true);
    });
  });
});
